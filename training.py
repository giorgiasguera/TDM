...
class TrainManager:
    ...
    def __init__(self, model: Model, config: dict, src_vocab: Vocabulary, test: bool = False):
        ...
        if not test:
            ...
            wandb.init(
                ...
                name="run_intero_paper",  # Il nome specifico di questo run
    ...
    def _train_batch(self, batch: dict, update: bool = True) -> Tensor:
        ...
        Identico a Sign-IDD/training.py → _train_batch().
        Modifiche:
          [3] batch è un dict (non un oggetto Batch)
          [4] normalizer usa len(batch["texts"]) invece di batch.nseqs,
              e int(batch["skel_lens"].sum()) invece di batch.ntokens

        :param batch:  dict aumentato da _prepare_batch()
        :param update: se True, aggiorna i pesi (gradient step)
        :return:       loss normalizzata
        """
    ...
    def _add_report(self, valid_score: float, valid_loss: float, eval_metric: str, new_best: bool = False, report_type: str = "val") -> None:
        ...
        Identico a Sign-IDD/training.py → _add_report().
        """
        current_lr = -1

...
def initialize_model(model: nn.Module, cfg: dict, src_padding_idx: int,
    ...
    # associa la funzione di inizializzazione corretta a ogni tipo di parametro (embedding, bias, weight)
    ...
    # per ogni parametro in base al suo tipo applica la funzione di inizializzazione corretta:
    with torch.no_grad():
        ...
        model.encoder.embedding.weight.data[src_padding_idx].zero_()
